...
export class Tag {
  ...
  private static readonly META_TAG_REGEX =
    /^meta:\s*(?<name>[^:\s]+)(\s*(?<value>.+))?$/;
  private static readonly TAG_REGEX = /^(?<name>[^:\s]+)(:?\s*(?<value>.+))?$/;
  private static readonly CUSTOM_META_TAG_REGEX =
    /^(?<name>x_[^:]+)(:?\s*(?<value>.+))?$/;
  ...
  public static parse(name: string): Tag | undefined {
    ...
    if (this.CUSTOM_META_TAG_REGEX.test(sanitizedName)) {
      const tag = this.parseRegex(sanitizedName, this.CUSTOM_META_TAG_REGEX);
      if (tag) {
    ...
    if (this.META_TAG_REGEX.test(sanitizedName)) {
      const tag = this.parseRegex(sanitizedName, this.META_TAG_REGEX);
      if (tag) {
    ...
    if (this.TAG_REGEX.test(sanitizedName)) {
      const tag = this.parseRegex(sanitizedName, this.TAG_REGEX);
      if (tag) {
  ...
  private static parseRegex(name: string, regex: RegExp): Tag | undefined {
    ...
    if (match && match.groups) {
      ...
      if (match.groups["value"]) {
        ...
        let attributeMatch;
        while (
          (attributeMatch = this.ATTRIBUTE_REGEX.exec(
            match.groups["value"],
          )) !== null
        ) {
          tag._attributes[attributeMatch[1]] =
